...
#genes <- readLines("C:\\Users\\Hugo Rodrigues\\Documents\\corkoakdataFIM\\Ensembl_Query.txt")
...
#Write Output Table
if (exists("outputname") == TRUE) {
  outputname <- paste(dir, outputname, sep = "/")
} else {
  outputname <- paste(dir, "Ensembl_Plants_Query_Output.txt", sep = "/")
}

outputname <- paste(dir, "Ensembl_Plants_Query_Output.txt", sep = "/")
print(outputname)
write.table(output, outputname, sep = "\t", row.names = FALSE, quote = FALSE)
